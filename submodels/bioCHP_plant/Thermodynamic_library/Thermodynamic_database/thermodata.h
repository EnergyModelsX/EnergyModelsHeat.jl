			...
			while( prop_set_found == false ){
				...
				//Deals with properties given in list form:
					...
					//If non of the above are read, we have moved on to the next element
						...
						td.lData.var.clear(); td.lData.P.clear(); td.min = -1e4; td.max = 1e4; td.lData.cons = 0.0;
				...
				//Deals with properties given in model form:
					...
					if( txt == "COMPOUND:" || txt != "COMPOUND") {compound_type_found = true;}
					if( txt != "COMPOUND:" || txt != "COMPOUND") {compound_type_found = false;}
					thermo.push_back(td); td.min = -1e4; td.max = 1e4;
					td.data_type = "VOID"; td.property_type = "VOID"; td.id="VOID"; td.function_dependence = "VOID";
					//clear the vectors:
...
bool extract_property(thermo_data &thermo, string property, double P, double T, double S, string input_file){
				...
				while(prop_set_found == false){
					...
					//Deals with properties given in list form:
						...
						//If non of the above are read, we have moved on to the next element
							...
							else if (td.function_dependence == "ENTROPY"){
								if(S>=td.min && S<=td.max){
									thermo =td; 
									property_found = true;
								}
							}								
...
void get_species_thermo_data(vector<thermo_data> &thermo, 
			     string species_id, 
			     string database_file = "Thermodynamic_library/Thermodynamic_database/Species_thermofile_list"){
	ifstream data_file;
...
bool get_property_data(thermo_data &thermo, string species_id, 
			string property_id, double P, double T, double S, 
			string database_file = "Thermodynamic_library/Thermodynamic_database/Species_thermofile_list"){
	ifstream data_file;
	data_file.open(database_file);
	...
	if (path_found == false){
		...
		thermo.id = id;
		bool found;
		found = extract_property(thermo, property_id, P, T, S, path);
		if(!found){
